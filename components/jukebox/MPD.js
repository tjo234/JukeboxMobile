...
const MPD_ADDR = 'raspberrypi.local';
const MPD_PORT = 6600;
...
class MPD {
  static connect() {
    const zeroconf = new EventEmitter(); // new Zeroconf();
    zeroconf.on('start', () => {
      alert('[Start]')
    })
    zeroconf.on('stop', () => {
...
// class MpdEmitter extends EventEmitter {}

// export function connect(){
//   let options = options || DEFAULT_OPTIONS;
//   const client = new MpdEmitter();
//   client = SocketConnection.connect(options, function() {
//     client.emit('connect');
//   });
